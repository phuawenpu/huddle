...
export default function ScenarioDetailPage() {
  ...
  const handleRevise = async () => {
    setAction(
      `Revising transcript with ${revisionPasses} LLM call${revisionPasses === 1 ? "" : "s"}`,
    );
    setActionError("");
    setRevisionSummary([]);
    try {
      const response = await fetch(
        `/api/scenarios/${params.scenarioId}/revise`,
        {
          method: "POST",
      ...
      if (!response.ok)
        throw new Error(body.error || "Transcript revision failed");
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="flex items-center gap-3 pt-4">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-hud-text">
              {scenario.title}
            </h1>
        ...
        <div className="bg-hud-surface border border-hud-border rounded-xl p-4">
          <h3 className="text-xs font-semibold text-hud-muted uppercase mb-1">
            Objective
          </h3>
            ...
            <div className="grid grid-cols-2 gap-2">
              {scenario.speakers?.map((s) => (
                <div key={s.index} className="bg-hud-bg rounded-lg p-3">
                  <div className="font-medium text-hud-text text-sm">
                    {s.name}
                  </div>
            ...
            <div className="grid grid-cols-3 gap-3 text-sm">
              <div>
                ...
                <p className="text-hud-text font-mono">
                  {scenario.budget.estimatedTurns}
                </p>
              </div>
              <div>
                <span className="text-hud-muted text-xs">Characters</span>
                <p className="text-hud-text font-mono">
                  {scenario.budget.estimatedCharacters?.toLocaleString()}
                </p>
              </div>
              <div>
                <span className="text-hud-muted text-xs">Cost</span>
                <p className="text-hud-text font-mono">
                  ${scenario.budget.estimatedCostUsd?.toFixed(2)}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-2xl font-mono text-hud-text mt-1">
                  {scenario.transcriptQuality.score}
                  <span className="text-sm text-hud-muted">/100</span>
                </p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs text-right">
                <div>
                  <span className="text-hud-muted">Overlap</span>
                  <p className="text-hud-text">
                    {scenario.transcriptQuality.overlapCount}
                  </p>
                </div>
                <div>
                  <span className="text-hud-muted">Reaction links</span>
                  <p className="text-hud-text">
                    {Math.round(
                      scenario.transcriptQuality.reactionCoverage * 100,
                    )}
                    %
                  </p>
                </div>
                <div>
                  <span className="text-hud-muted">Round-robin</span>
                  <p className="text-hud-text">
                    {Math.round(
                      scenario.transcriptQuality.roundRobinRatio * 100,
                    )}
                    %
                  </p>
                </div>
                <div>
                  <span className="text-hud-muted">Planned wpm</span>
                  <p className="text-hud-text">
                    {scenario.transcriptQuality.plannedWordsPerMinute ?? "—"}
                  </p>
                </div>
              </div>
            </div>
            {(scenario.transcriptQuality.errors.length > 0 ||
              scenario.transcriptQuality.warnings.length > 0) && (
              <ul className="mt-3 space-y-1 text-sm">
                {scenario.transcriptQuality.errors.map((message, index) => (
                  <li key={`error-${index}`} className="text-hud-danger">
                    • {message}
                  </li>
                ))}
                {scenario.transcriptQuality.warnings.map((message, index) => (
                  <li key={`warning-${index}`} className="text-hud-warn">
                    • {message}
                  </li>
                ))}
              </ul>
        ...
        {scenario.preflight && (
          ...
          >
            <h3
              ...
              style={{
                color: scenario.preflight.passed
            ...
            {!scenario.preflight.passed &&
              scenario.preflight.mergedPairs?.length > 0 && (
                <p className="text-sm text-hud-text">
                  Merged speakers:{" "}
                  {scenario.preflight.mergedPairs
                    .map(
                      ([a, b]) =>
            ...
            {scenario.preflight.speechValidation && (
              <p className="text-xs text-hud-muted mt-2">
                Audio check:{" "}
                {scenario.preflight.speechValidation.method.replace(/_/g, " ")}
                {scenario.preflight.speechValidation.sampledTurnCount > 0 && (
                  <>
                    {" "}
                    · {
                      scenario.preflight.speechValidation.sampledTurnCount
                    }{" "}
                    clips · average WER{" "}
                    {Math.round(
                      (scenario.preflight.speechValidation
                        .averageWordErrorRate || 0) * 100,
                    )}
                    %
                  </>
                )}
        ...
        {scenario.turns != null && scenario.speakers != null && (
          <section className="bg-hud-surface border border-hud-border rounded-xl p-4 space-y-4">
            <div>
              <h3 className="text-xs font-semibold text-hud-muted uppercase">
                Transcript workshop
              </h3>
              <p className="text-sm text-hud-muted mt-1">
                Run one focused pass or chain up to three calls. Each call
                receives the full speaker-, timing-, reaction-, and
                overlap-aware transcript plus the preceding result.
              </p>
            </div>
            <div className="grid gap-3 md:grid-cols-[1fr_160px]">
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setRevisionPreset(
                      event.target.value as typeof revisionPreset,
                    )
                  }
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setRevisionPasses(Number(event.target.value))
                  }
                  className="w-full bg-hud-bg border border-hud-border rounded-lg px-3 py-2.5 text-sm"
                >
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                disabled={
                  Boolean(action) ||
                  (revisionPreset === "custom" && !revisionInstruction.trim())
                }
              ...
              <p className="text-xs text-hud-warn">
                A successful edit invalidates the old mix; synthesize and
                preflight again.
              </p>
            </div>
            {revisionSummary.length > 0 && (
              <div className="rounded-lg border border-hud-success/30 bg-hud-success/10 p-3">
                <p className="text-xs font-semibold text-hud-success uppercase">
                  Revision complete
                </p>
                <ol className="mt-1 space-y-1 text-sm text-hud-text">
                  {revisionSummary.map((summary, index) => (
                    <li key={index}>
                      {index + 1}. {summary}
                    </li>
                  ))}
          ...
          <div className="bg-hud-surface border border-hud-border rounded-xl p-4">
            ...
            <div className="space-y-1 max-h-[38rem] overflow-y-auto">
              {scenario.turns?.map((t) => {
                const speaker = scenario.speakers?.find(
                  (s) => s.index === t.speakerIndex,
                );
                const anchor = t.overlap
                  ? scenario.turns?.find(
                      (candidate) =>
                        (candidate.id || `t${candidate.index}`) ===
                        t.overlap?.withTurnId,
                    )
                  : null;
                const anchorSpeaker = anchor
                  ? scenario.speakers?.find(
                      (candidate) => candidate.index === anchor.speakerIndex,
                    )
                  : null;
                return (
                  <div
                    key={t.id || t.index}
                    className="grid grid-cols-[76px_1fr] gap-3 py-3 border-b border-hud-border/30 last:border-0"
                  >
                    <div className="text-xs font-mono">
                      <p className="text-hud-accent">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-1.5 text-[10px]">
                        {t.isCalibration && (
                          <span className="text-hud-muted">calibration</span>
                        )}
                        {t.expectedCategory && (
                        ...
                        {t.expected?.reactsToTurnId && (
                          <span className="text-hud-muted">
                            ↳ responds to {t.expected.reactsToTurnId}
                          </span>
                        )}
                        {t.overlap && (
                          <span className="text-hud-warn">
                            ⟷ {t.overlap.kind} · starts{" "}
                            {t.overlap.startBeforeEndMs ??
                              t.overlap.startOffsetMs ??
                              0}
                            ms before{" "}
                            {anchorSpeaker?.name || t.overlap.withTurnId} ends ·{" "}
                            {t.overlap.resolution || "resolves"}
                          </span>
                        )}
                        {t.delivery && (
                          <span className="text-hud-muted">
                            {t.delivery.pace} · {t.delivery.volume} ·{" "}
                            {t.delivery.disfluency.replace("_", " ")}
                          </span>
                        )}
                      </div>
                    </div>
