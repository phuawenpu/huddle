...
describe("version-2 scenario transcripts", () => {
  ...
  it("round-trips relative overlap and delivery while clearing realized timing on revision", () => {
    const turns = normalizeScenarioTurns(
      [
        { id: "t0", speakerIndex: 0, text: "The retry action disappears after the error.", startMs: 100, endMs: 1900 },
        {
          id: "t1",
          speakerIndex: 1,
          ...
          timing: {
            gapBeforeMs: 0,
            overlap: { withTurnId: "t0", startBeforeEndMs: 520, kind: "interruption", resolution: "yield" },
            realizedStartMs: 1380,
            realizedEndMs: 2600,
          },
          delivery: { pace: "quick", tone: "checking a distinction", volume: "normal", disfluency: "light" },
          dialogue: { act: "questions", substantive: true, respondsToTurnId: "t0", intent: "clarify the failure condition" },
        },
        {
          id: "t2",
          speakerIndex: 0,
    ...
    const document = toEditableTranscript("Retry flow", "Find the next test", speakers, turns);
  ...
  it("fingerprints authored audio inputs but ignores measured timestamps", () => {
    const base = normalizeScenarioTurns(
      [
        { id: "t0", speakerIndex: 0, text: "The evidence is in the retry logs.", pauseBeforeMs: 300 },
        { id: "t1", speakerIndex: 2, text: "Then let’s inspect failures before changing the screen.", pauseBeforeMs: 420 },
        { id: "t2", speakerIndex: 0, text: "Agreed; I’ll bring the timeout cases into tomorrow’s test.", pauseBeforeMs: 260 },
      ],
      speakers.length
    );
